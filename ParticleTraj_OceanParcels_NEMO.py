#/**************************************************************************************/  
#//  Reads oceanic data from a NEMO file and computes particle trajectories
#//  with a settling velocity.
#//  Software provided by OceanParcels: https://doi.org/10.5194/gmd-12-3571-2019
#//  In this case study, the input file is generated from NEMO in the Mediterranean sea.
#//  Related work: hhttps://doi.org/10.5194/os-17-431-2021
#/**************************************************************************************/  
...
def Velocity(particle, fieldset, time):
...
def BrownianDiffusion(particle, fieldset, time):
    #GeneralParameters
    (uaux, vaux, waux) = fieldset.UVW[time, particle.depth, particle.lat, particle.lon] #
    if(uaux>0 or uaux<0 or vaux>0 or vaux<0 or waux>0 or waux<0):
       pi = math.acos(-1.0)
       rads = math.pi / 180
       degrees = 180 / math.pi
       rearth = 6371000
...
data = np.loadtxt('particles005', dtype=np.str)
lon = data[:,0] 
lat = data[:,1] 
initial_depth = 1
...
simdays=30
pset = ParticleSet(fieldset=fieldset, pclass=ParticleVariables, lon=lon, lat=lat, depth=depths, time=times)

kernels = pset.Kernel(Velocity) + pset.Kernel(WindStress) + pset.Kernel(MaxeyRiley3) + pset.Kernel(BrownianDiffusion) 

outdir = '/scratch/rebeca/output_S1/'
outfile = outdir+ 'sim2b099.nc'

pset.execute(kernels,runtime=timedelta(days=simdays),dt=timedelta(minutes=0.1),
             recovery={ErrorCode.ErrorOutOfBounds: DeleteParticle},
             output_file=pset.ParticleFile(name=outfile, outputdt=timedelta(hours=2)))
